...
class OpenImages(Dataset):
    ...
    def __getitem__(self, item):
        ...
        radius = self.radius
        crop_size = 224
        # print(width,height,crop_size)
        if width<1.2*crop_size or height<1.2*crop_size:
            crop_size = min(width,height)/2
            print(crop_size, width - crop_size)
            print(crop_size, height - crop_size)
        # print(width-crop_size,height-crop_size)
        crop_size = int(crop_size)
        width1 = random.randint(int(crop_size), int(width))
        height1 = random.randint(int(crop_size), int(height))
        ...
        for i in range(1,10000):
            if self.is_safe(width1,height1,width2,height2,width,height):
                break
            else:
                width1 = random.randint(int(crop_size), int(width))
                height1 = random.randint(int(crop_size), int(height))
                ...
                width2 = width1 + crop_size
                height2 = height1 + crop_size
        # print(self.is_safe(width1, height1, width2, height2, width, height))

        # if not self.is_safe(width1,height1,width2,height2,width,height):
        #     while(self.is_safe(width1,height1,width2,height2,width,height)==False):
        #         width1 = random.randint(int(crop_size), int(width))
        #         height1 = random.randint(int(crop_size), int(height))
        #
        #         width2 = width1 + crop_size
        ...
        for x in range(0,number_of_points+1):
            ...
            width1_new_point = math.cos(2*pi/number_of_points*random.randint(0,number_of_points))*radius
            height1_new_point = math.sin(2 * pi / number_of_points * random.randint(0,number_of_points)) * radius
            ...
            # pdb.set_trace()
            if self.is_safe(width1_new_point_final,height1_new_point_final,width2_new_point,height2_new_point,width_crop,height_crop):
                # print("here")
                # pdb.set_trace()


                crop_box = (int(width1_new_point_final), int(height1_new_point_final), int(width2_new_point), int(height2_new_point))
            ...
            if self.is_safe(width2_new_point, height2_new_point, width2_new_point_2, height2_new_point_2,width_crop,height_crop):
                # print("here2")
                crop_box = (int(width2_new_point_2),int(height2_new_point_2),int(width2_new_point), int(height2_new_point))
                # print(crop_box)
