...
from openimages_dataset_test import OpenImages as OpenImagesTestLoader
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            }, is_best,save_name=args.save_name)
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        # target = target.to(device=args.gpu, dtype=torch.int64)
        ...
        m_ap, w_ap, m_aps = map(output, target,{})
        # import pdb
        # pdb.set_trace()
        losses.update(loss.item(), images.size(0))
...
def save_checkpoint(state, is_best,save_name, filename='checkpoint_lincls_with_fulldataset_objects_2_epoch.pth.tar'):
...
def map(submission_array, gt_array,all_class_map):
    ...
    #     if not np.isnan(m):
    #         # pdb.set_trace()
    #         all_class_map[i] = all_class_map[i] + m
    #     i = i + 1
